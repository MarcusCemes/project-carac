...
def process_dataframe(df: DataFrame) -> None:
    ...
    add_aero_forces(df, DF_FORCE, DF_AERO_FORCES)

    if ANALYTICAL_MODELLING and has_drone_actuation(df):
        add_analytical_model(df)
        add_aero_forces(df, DF_FORCE_MODEL, DF_AERO_FORCES_MODEL)
...
def add_body_frame_kinematics(df: DataFrame) -> None:
    ...
    rot = Rotation.from_euler(EULER_SEQ, df[WF_ATTITUDE].to_numpy())

    # Save the quaternion representation of the drone's attitude (x, y, z, w)
    df[WF_QUATERNION] = rot.as_quat(scalar_first=False)
    ...
    positions = df[WF_POSITION].to_numpy()
...
def add_aero_forces(
    ...
    alpha, beta = df[DF_AERO_ANGLES].to_numpy().T
...
def add_analytical_model(df: DataFrame) -> None:
    ...
    positions = df[WF_POSITION].to_numpy()
    attitudes = df[WF_QUATERNION].to_numpy()
    angular_velocities = df[DF_PQR].to_numpy()
    relative_velocities = df[DF_UVW].to_numpy()
    drone_actuators = df[DRONE_ACT].to_numpy()
    ...
    df[DF_FORCE_MODEL] = forces_list
    df[DF_MOMENT_MODEL] = moments_list
...
def has_drone_actuation(df: DataFrame) -> bool:
    ...
    return (
        all(col in df.columns for col in DRONE_ACT)
        and not df[DRONE_ACT].isnull().all().all()
    )
